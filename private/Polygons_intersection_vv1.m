function [Geo] = Polygons_intersection_vv1(S,Display_result,Accuracy)
...
%         %plot(polyshape(S(i).P.y,S(i).P.x),'FaceColor',[0.7, 0.1, 0.1],'FaceAlpha',0.3); hold on
%         %plot(polyshape(S(j).P.y,S(j).P.x),'FaceColor',[0.7, 0.1, 0.1],'FaceAlpha',0.3); hold off
%         %P = PolygonClip(S(i).P,S(j).P,1);
%         P = intersect(S(i).P,S(j).P);
%         P = regions(P);
%             
...
%             for p = 1:size(P,2)
%               Res(i,j) =Res(i,j) + P(p).area; %(1-2*P(p).hole)*polyarea(P(p).x,P(p).y);
%             end
...
%         P1 = polyshape(Geo(V(i)).P.x,Geo(V(i)).P.y);%P1.plot;hold on
%         P2 = polyshape(S(Index2(i).data(end)).P.x,S(Index2(i).data(end)).P.y);%P2.plot;hold off
...
Geo = Polygons_intersection_Posttreatment_vv(S,Geo,Display_result,Accuracy);
