function Geo = Polygons_intersection_Posttreatment_vv(S,Geo,Display_result,...
                                                                  Accuracy)
...
Geo   = [S,Geo];
Geo   = Polygons_intersection_Polygon_cleanup_vv(Geo,Accuracy);
N_Geo =  numel(Geo);
...
%                 end
                ...
                P = Polygons_intersection_Polygon_cleanup_vv(P,Accuracy);
                if ~isempty(P)
                    Geo(Mod(k)).P = P;
                else
...
[Geo, Geo_area] = Polygons_intersection_Compute_area_vv(Geo);
N_Geo = numel(Geo);
To_delete = [];
for i=1:N_Geo  
    for j=1:numel(Geo(i).P)
